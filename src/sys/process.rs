use core::ffi::{c_long,c_void, c_ulong};

use crate::uuid::Uuid;

use super::{handle::{Handle,HandlePtr}, kstr::{KStrCPtr, KStrPtr}, permission::SecurityContext, result::SysResult, fs::FileHandle};
...
extern "C"{
    ...
    pub fn GetEnvironmentVariable(hdl: HandlePtr<EnvironmentMapHandle>, name: KStrCPtr, val_out: *mut KStrPtr) -> SysResult;
    /// Writes the given environment handle, with a given variable name and value.
    pub fn SetEnvironmentVariable(hdl: HandlePtr<EnvironmentMapHandle>, name: KStrCPtr, val: KStrCPtr) -> SysResult;
    ...
    pub fn CopyEnvironment(hdl: *mut HandlePtr<EnvironmentMapHandle>, map: HandlePtr<EnvironmentMapHandle>) -> SysResult; 
    ...
    pub fn CreateProcess(ctx: *const ProcessStartContext, hdl: *mut HandlePtr<ProcessHandle>) -> SysResult;

    ///
    /// Enumerates over the list of processes on the system
    pub fn EnumerateProcesses(hdl: *mut HandlePtr<EnumerateProcessHandle>, flags: u32) -> SysResult;

    /// Advances the enumeration list
    pub fn EnumerateNext(hdl: HandlePtr<EnumerateProcessHandle>, state: *mut *mut c_void) -> SysResult;

    /// Reads from the current pointer in the EnumerateProcessHandle
    pub fn EnumerateRead(hdl: HandlePtr<EnumerateProcessHandle>, state: *mut c_void, info: *mut ProcessInfo) -> SysResult;
}