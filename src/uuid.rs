...
pub const fn parse_uuid(st: &str) -> Uuid{
    match try_parse_uuid(st){
        ...
        Err(TryParseUuidError::InvalidDigit(_)) => panic!("Encountered an invalid character where a hex digit is expected"),
        Err(TryParseUuidError::UnexpectedChar(_)) => panic!("Encountered an invalid character in the UUID"),
    }
}